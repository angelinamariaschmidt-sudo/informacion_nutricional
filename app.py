...
with tab1:
    ...
    # Búsqueda en FatSecret complementaria
    with st.expander("🔍 ¿No encontrás un ingrediente comercial? Buscalo en FatSecret Argentina"):
        col_busq1, col_busq2 = st.columns([3, 1])
        with col_busq1:
            termino_busqueda = st.text_input("Ingrediente o marca comercial:", placeholder="Ej: gluten, avena, premezcla...")
        with col_busq2:
            st.write("")
            st.write("")
            if termino_busqueda:
                termino_codificado = urllib.parse.quote(termino_busqueda)
                url_fatsecret = f"https://www.fatsecret.com.ar/calor%C3%ADas-nutrici%C3%B3n/search?q={termino_codificado}"
                st.link_button("🌐 Buscar en FatSecret", url_fatsecret)
            else:
                st.link_button("🌐 Abrir FatSecret", "https://www.fatsecret.com.ar/calor%C3%ADas-nutrici%C3%B3n/")

    # Carga desde Base Interna SARA 2
    col1, col2, col3 = st.columns([3, 2, 1])
    with col1:
        ing_elegido = st.selectbox("Elegí de la base precargada (SARA 2 / FatSecret):", lista_alimentos_ordenada)
    with col2:
    ...
    # Formulario rápido para incorporar cualquier ingrediente hallado en FatSecret
    with st.expander("➕ Cargar ingrediente manual o desde FatSecret (cada 100 g)"):
        ...
        if st.button("📥 Incorporar ingrediente manual"):
            if nombre_manual.strip():
                ...
                st.success(f"'{nombre_manual}' añadido a la formulación.")
                st.rerun()
            else:
                st.warning("Completá el nombre del ingrediente.")

    # Formulación actual por defecto
    if "receta" not in st.session_state:
        st.session_state.receta = [
